...
async function refresh(){data=await api('/api/state');$('#nEdges').textContent=data.edges.filter(x=>x.state==='ready').length;$('#nTenants').textContent=data.tenants.length;$('#nDns').textContent=data.dns_records.filter(x=>x.status==='active').length;$('#webPort').value=data.web_port;$('#health').innerHTML=rows(['Edge','IP','Estado','Ações'],data.edges.map(x=>`<tr><td>${x.name}</td><td>${x.ipv4}</td><td class="status ${x.state}">${x.state}</td><td><button onclick="health('${x.id}')">Testar</button></td></tr>`));$('#edgeList').innerHTML=rows(['Nome','IP','Status',''],data.edges.map(x=>`<tr><td>${x.name}</td><td>${x.ipv4}</td><td class="${x.state}">${x.state}</td><td><button class="danger" onclick="drain('${x.id}')">Drenar</button></td></tr>`));$('#tenantList').innerHTML=rows(['ID','Host','Versão',''],data.tenants.map(x=>`<tr><td>${x.id}</td><td>${x.canonical_host}</td><td>${x.config_version}</td><td><button onclick="vhost('${x.id}')">Ver vhost</button></td></tr>`));$('#tenantSelect,#vodTenant').innerHTML=data.tenants.map(x=>`<option value="${x.id}">${x.id}</option>`).join('');$('#domainList').innerHTML=rows(['Hostname','Tenant','SSL'],data.tenants.flatMap(x=>x.hosts.map(h=>`<tr><td>${h.hostname}</td><td>${x.id}</td><td class="${h.tls_status}">${h.tls_status}</td></tr>`)));let vus=data.tenants.flatMap(x=>x.upstreams.filter(u=>u.kind==='vod').map(u=>({tenant:x.id,...u})));$('#vodList').innerHTML=rows(['Tenant','Fonte','Porta','Ações'],vus.map(u=>`<tr><td>${u.tenant}</td><td>${u.host}</td><td>${u.port}</td><td><button class="secondary" onclick="editVod('${u.id}','${u.host}',${u.port})">Editar</button> <button class="danger" onclick="deleteVod('${u.id}')">Apagar</button></td></tr>`));$('#dnsList').innerHTML=rows(['Hostname','Tipo','Destino','Status'],data.dns_records.map(x=>`<tr><td>${x.hostname}</td><td>${x.record_type}</td><td>${x.target_ip}</td><td class="${x.status}">${x.status}</td></tr>`))}
