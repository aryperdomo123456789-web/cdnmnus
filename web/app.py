...
const csrf=document.querySelector('meta[name=csrf]').content,$=s=>document.querySelector(s);const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));let data={};let refreshBusy=false;
...
const api=async(path,options={})=>{options.headers={...(options.headers||{}),'X-CSRF-Token':csrf};if(options.body)options.headers['Content-Type']='application/json';let r=await fetch(path,options),text=await r.text(),j={};try{j=text?JSON.parse(text):{}}catch(_){j={}}if(!r.ok)throw Error(j.error||`HTTP ${r.status}`);return j};
...
async function refresh(){data=await api('/api/state');$('#nEdges').textContent=data.edges.filter(x=>x.state==='ready').length;$('#nTenants').textContent=data.tenants.length;$('#nDns').textContent=data.dns_records.filter(x=>x.status==='active').length;$('#webPort').value=data.web_port;$('#health').innerHTML=rows(['Edge','IP','Estado','Ações'],data.edges.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.ipv4)}</td><td class="status ${esc(x.state)}">${esc(x.state)}</td><td><button onclick="health('${esc(x.id)}')">Testar</button></td></tr>`));$('#edgeList').innerHTML=rows(['Nome','IP','Status',''],data.edges.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.ipv4)}</td><td class="${esc(x.state)}">${esc(x.state)}</td><td><button class="danger" onclick="drain('${esc(x.id)}')">Drenar</button></td></tr>`));$('#tenantList').innerHTML=rows(['ID','Host','Versão',''],data.tenants.map(x=>`<tr><td>${esc(x.id)}</td><td>${esc(x.canonical_host)}</td><td>${esc(x.config_version)}</td><td><button onclick="vhost('${esc(x.id)}')">Ver vhost</button></td></tr>`));$('#tenantSelect,#vodTenant').innerHTML=data.tenants.map(x=>`<option value="${esc(x.id)}">${esc(x.id)}</option>`).join('');$('#domainList').innerHTML=rows(['Hostname','Tenant','SSL'],data.tenants.flatMap(x=>x.hosts.map(h=>`<tr><td>${esc(h.hostname)}</td><td>${esc(x.id)}</td><td class="${esc(h.tls_status)}">${esc(h.tls_status)}</td></tr>`)));let vus=data.tenants.flatMap(x=>x.upstreams.filter(u=>u.kind==='vod').map(u=>({tenant:x.id,...u})));$('#vodList').innerHTML=rows(['Tenant','Fonte','Porta','Ações'],vus.map(u=>`<tr><td>${esc(u.tenant)}</td><td>${esc(u.host)}</td><td>${esc(u.port)}</td><td><button class="secondary" onclick="editVod('${esc(u.id)}','${esc(u.host)}',${esc(u.port)})">Editar</button> <button class="danger" onclick="deleteVod('${esc(u.id)}')">Apagar</button></td></tr>`));$('#dnsList').innerHTML=rows(['Hostname','Tipo','Destino','Status'],data.dns_records.map(x=>`<tr><td>${esc(x.hostname)}</td><td>${esc(x.record_type)}</td><td>${esc(x.target_ip)}</td><td class="${esc(x.status)}">${esc(x.status)}</td></tr>`))}
...
$('#vodForm').onsubmit=async e=>{e.preventDefault();let f=Object.fromEntries(new FormData(e.target));f.port=Number(f.port);try{await api('/api/vod-sources',{method:'POST',body:JSON.stringify(f)});await refresh();notify('Fonte VOD cadastrada. Gere o deploy para publicar.')}catch(x){notify(x.message,true)}};window.editVod=async(id,host,port)=>{let h=prompt('Domínio VOD',host),p=prompt('Porta',port);if(!h||!p)return;try{await api(`/api/vod-sources/${id}`,{method:'PATCH',body:JSON.stringify({host:h,port:Number(p)})});await refresh();notify('Fonte VOD atualizada.')}catch(x){notify(x.message,true)}};window.deleteVod=async id=>{if(!confirm('Apagar esta fonte VOD?'))return;try{await api(`/api/vod-sources/${id}`,{method:'DELETE'});await refresh();notify('Fonte VOD removida. Gere o deploy para aplicar.')}catch(x){notify(x.message,true)}};
...
$('#portForm').onsubmit=async e=>{e.preventDefault();try{await api('/api/settings/port',{method:'POST',body:JSON.stringify({port:Number(e.target.port.value)})});notify('Porta salva. Reinicie o painel para aplicar.')}catch(x){notify(x.message,true)}};const refreshSafe=async()=>{if(refreshBusy)return;refreshBusy=true;try{await refresh()}catch(e){notify(e.message,true)}finally{refreshBusy=false}};refreshSafe();setInterval(refreshSafe,30000);
</script></body></html>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if route == "/api/playback/feature-flag":
                tenant_id = str(payload.get("tenant_id", ""))
                enabled = bool(payload.get("enabled", False))
                tenant = DB.set_playback_sessions_enabled(tenant_id, enabled)
                self._json(200, {"tenant": tenant, "playback_sessions_v1": bool(tenant.get("playback_sessions_v1", 0))}); return
    ...
    def _upstream_mutation(self, method: str) -> None:
        if not self._auth(): return
        try:
            route = urlsplit(self.path).path
            parts = route.strip("/").split("/")
            if len(parts) != 3 or parts[:2] != ["api", "vod-sources"]:
                self._json(404, {"error": "rota não encontrada"}); return
            payload = self._payload() if method == "PATCH" else {}
            if method == "PATCH":
                self._json(200, {"upstream": DB.update_upstream(parts[2], str(payload.get("host", "")), int(payload.get("port", 80)))})
            else:
                DB.delete_upstream(parts[2]); self._json(200, {"deleted": parts[2]})
        except PermissionError as exc:
            self._json(403, {"error": str(exc)})
        except Exception as exc:
            self._json(400, {"error": str(exc)})
    ...
    def do_PATCH(self) -> None:
        self._upstream_mutation("PATCH")

    def do_DELETE(self) -> None:
        self._upstream_mutation("DELETE")
