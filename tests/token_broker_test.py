...
real_http_connection = mod.http.client.HTTPConnection
try:
    ...
    class FakeConnection:
        ...
        def __init__(self, host, *args, **kwargs):
            FakeConnection.connected_to = host
    ...
    try:
        ...
        raise AssertionError("SSRF aceito")
    except PermissionError:
        pass
    location = "http://lb.test:8080/token"
    try:
        real_query("/hls/20_2.ts", cfg)
        raise AssertionError("porta aceita")
    except ValueError:
        pass
    location = "https://lb.test/token"
    try:
        real_query("/hls/20_2.ts", cfg)
        raise AssertionError("HTTPS incompatível aceito")
    except ValueError:
        pass
    ...
    try:
        mod.safe_request_uri("/movie/u/p/99.mp4")
        raise AssertionError("VOD aceito como HLS")
    except ValueError:
        pass
    ...
    class VodResponse(FakeResponse):
        def __init__(self):
            self.status, self.location = responses.pop(0)
    ...
    mod.query_origin = real_query
    mod.http.client.HTTPConnection = real_http_connection
    server.shutdown()
    tmp.cleanup()
