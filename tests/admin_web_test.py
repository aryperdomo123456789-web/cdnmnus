...
mod.DB = mod.Database(Path(temp.name) / "admin.db")
mod.DB.initialize()
...
def request(method, path, payload=None, csrf=True, authenticated=True):
    ...
    if payload is not None:
        ...
        if csrf:
            headers["X-CSRF-Token"] = mod.CSRF_TOKEN
...
try:
    ...
    server.server_close()
    temp.cleanup()
    if _original_admin_password is None:
        os.environ.pop("CDNMNUS_ADMIN_PASSWORD", None)
    else:
        os.environ["CDNMNUS_ADMIN_PASSWORD"] = _original_admin_password

print("admin web/auth/csrf/config checks: OK")
